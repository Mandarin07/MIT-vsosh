...
class SnapshotManager:
    ...
    def _recv(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            # Try to parse JSON
            try:
                # Skip event messages, wait for response
    ...
    def _execute(self, command: str, arguments: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    def create_snapshot(self, name: str, description: str = "") -> SnapshotInfo:
        """
        Create a VM snapshot (savevm).
        
        This saves the complete VM state (memory, CPU, devices) to
        the qcow2 image. Takes 2-5 seconds depending on RAM size.
        ...
        try:
            # Use human-monitor-command for savevm (not available in QMP directly)
...
class ExternalSnapshotManager:
    ...
    def list_overlays(self) -> List[str]:
        ...
        for f in os.listdir(self.snapshots_dir):
            if f.endswith('.qcow2'):
                overlays.append(f[:-6])  # Remove .qcow2 extension
        return overlays
